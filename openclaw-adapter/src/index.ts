  ...
  try {
    ...
    if (errors.length > 0) {
      ...
      try {
        ...
        // Force-start the health loop — OpenClaw may not call start() on the service
        manager.start().catch((err: unknown) => {
          log.warn?.("[unwind] manual manager.start() failed:", err);
        });
        log.info?.("[unwind] health loop force-started");
      } catch (err) {
