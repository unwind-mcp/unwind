...
# Safe sync: Mac -> Pi (non-destructive by default)
#
# Usage:
#   ./tools/sync-to-pi.safe.sh [--apply] [--prune] [--yes]
#
# Defaults:
...
Examples:
  ./tools/sync-to-pi.safe.sh
  ./tools/sync-to-pi.safe.sh --apply
  ./tools/sync-to-pi.safe.sh --apply --prune
EOF
}
...
pytest_count_remote() {
  remote "if [ -x '$PI_PATH/.venv/bin/python' ] && [ -f '$PI_PATH/pyproject.toml' ]; then cd '$PI_PATH' && .venv/bin/python -m pytest --collect-only 2>/dev/null | awk '/tests collected/{print \\$1}' | tail -1; else echo n/a; fi"
}
...
echo "[SYNC] Direction: Mac -> Pi"
echo "[SYNC] Source:    $MAC_PATH"
echo "[SYNC] Target:    ${PI_USER}@${PI_IP}:$PI_PATH"
if (( APPLY == 1 )); then
  if (( PRUNE == 1 )); then
...
echo "[PRECHECK]"
if [[ ! -d "$MAC_PATH" ]]; then
  echo "Source path not found: $MAC_PATH"
  exit 1
fi

echo "  - Source path exists: YES"
remote "echo ok" >/dev/null 2>&1 || {
  echo "  - Target reachable: NO (ssh failed)"
  exit 1
}
echo "  - Target reachable: YES"
...
rsync "${RSYNC_ARGS[@]}" "$MAC_PATH" "${PI_USER}@${PI_IP}:$PI_PATH" | tee "$DRYRUN_OUT"
...
if (( PRUNE == 1 )); then
  ...
  TS="$(date +%Y%m%d-%H%M%S)"
  BACKUP_PATH="${PI_PATH%/}/.sync-backups/${TS}-before-prune.tar.gz"
  remote "mkdir -p '${PI_PATH%/}/.sync-backups'"
  remote "tar -czf '$BACKUP_PATH' --exclude='.git' --exclude='.sync-backups' -C '$PI_PATH' ."
...
rsync "${APPLY_ARGS[@]}" "$MAC_PATH" "${PI_USER}@${PI_IP}:$PI_PATH"

echo "[APPLY] Complete."

echo ""
echo "[INTEGRITY]"
SRC_COUNT="$(count_local_files "$MAC_PATH")"
DST_COUNT="$(count_remote_files)"
SRC_HEAD="$(git_head_local)"
DST_HEAD="$(git_head_remote)"
DST_TESTS="$(pytest_count_remote)"
