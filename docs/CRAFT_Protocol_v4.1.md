# CRAFT: Cryptographic Relay Authentication for Faithful Transmission (v4.1)

**Author:** David Russell (Brug.AI)
**Contributing reviewers:** SENTINEL, three external adversarial collaborators
**Date:** February 2026
**Status:** Working Draft v4.1 (post collab-3 round-2 fixes)
...
PRK_cap_root = HKDF-Extract(salt = "CRAFT/v4/cap-root", IKM = server_secret)
K_cap_srv  = HKDF-Expand(PRK_cap_root, info = "CRAFT/v4/cap\0" || len(ctx) || ctx, L=32)
...
For envelope MAC input construction (`MAC_input(envelope)`), the following byte-level rules apply:

- Build `MAC_input` from the envelope with both `mac` and `state_commit` removed.
- `MAC_input` MUST include: `v`, `epoch`, `session_id`, `account_id`, `channel_id`, `conversation_id`, `context_type`, `seq`, `msg_type`, `direction`, `ts_ms`, and `payload`.
...
- Variable-length field concatenation MUST use length-prefixed encoding: 4-byte big-endian length prefix followed by field bytes.
...
All directional keys (`K_msg_*`, `K_state_*`, `K_resync_*`) are re-derived from `PRK_new` using the same HKDF-Expand labels with the updated epoch in the info string. `K_cap_srv` is re-derived from `PRK_cap_root` with updated epoch context.
...
- `server_secret` MUST be generated from a CSPRNG with at least 256 bits of entropy.
...
```text
Binary `MAC_input(envelope)` as defined in Section 4.4: length-prefixed concatenation of canonical field bytes with `mac` and `state_commit` excluded, including `seq` as uint64 big-endian and payload as UTF-8 bytes of JCS-canonical JSON.
```
...
This version (v4.1) incorporates findings from three independent adversarial reviews (plus collab-3 round-2) conducted in February 2026. Key changes from v3 and their provenance:
