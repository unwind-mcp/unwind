...
- `session_id` MUST be generated by P as: `session_id = base64url_no_pad(CSPRNG(16 bytes))`.
- `session_id` MUST be globally unique for active sessions.
...
All protocol string literals used in cryptographic operations (HKDF labels/salts, HMAC labels, control action labels) MUST be UTF-8 encoded bytes.
...
- After epoch switch, P MUST reject any envelope bearing `epoch_old`, even if its `seq` would otherwise be valid for that old epoch. Return `ERR_EPOCH_STALE` (post-MAC only).
...
Example wire envelope (complete):
...
MAC input object (before UTF-8 encoding):

```json
{"account_id":"acct_001","channel_id":"ch_main","context_type":"dm","conversation_id":"conv_7f3a","direction":"c2p","epoch":0,"msg_type":"user_instruction","payload":{"meta":{},"text":"delete file /tmp/test.txt"},"seq":"42","session_id":"sess_abc123","ts_ms":1739999999123,"v":4}
```

MAC input bytes (for K_msg_c2p):

```text
UTF-8 bytes of the JCS-canonical JSON object above (Option A, Section 4.4).
```
